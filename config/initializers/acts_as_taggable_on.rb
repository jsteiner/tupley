      ...
      def tagged_with_slugs(tags, options = {})
        ...
        if options.delete(:exclude)
          if options.delete(:wild)
            tags_conditions = tag_list.map { |t| sanitize_sql(["#{ActsAsTaggableOn::Tag.table_name}.slug #{like_operator} ? ESCAPE '!'", "%#{escape_like(t)}%"]) }.join(" OR ")
          else
            tags_conditions = tag_list.map { |t| sanitize_sql(["#{ActsAsTaggableOn::Tag.table_name}.slug #{like_operator} ?", t]) }.join(" OR ")
          end
          ...
          if options.delete(:wild)
            ...
            tags = ActsAsTaggableOn::Tag.slugged_any(tag_list)
          end
          ...
          taggings_alias   = adjust_taggings_alias(
            "#{alias_base_name[0..4]}#{taggings_context[0..6]}_taggings_#{sha_prefix(tags.map(&:slug).join('_'))}"
          )
          ...
          tags.each do |tag|
            ...
            taggings_alias = adjust_taggings_alias("#{alias_base_name[0..11]}_taggings_#{sha_prefix(tag.slug)}")
